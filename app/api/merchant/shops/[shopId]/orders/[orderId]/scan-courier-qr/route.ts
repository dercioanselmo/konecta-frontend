...
export async function POST(request: Request, ctx: RouteContext<"/api/merchant/shops/[shopId]/orders/[orderId]/scan-courier-qr">) {
  const token = await requireAccessToken();
  if (token instanceof NextResponse) return token;
  const { shopId, orderId } = await ctx.params;
  try {
    const order = await courierApiFetch<MerchantOrder>(`/api/v1/merchant/shops/${shopId}/orders/${orderId}/scan-courier-qr`, {
      method: "POST",
